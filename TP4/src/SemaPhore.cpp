...
SemaphoreProducer::SemaphoreProducer(Semaphore *semaphore, unsigned int maxCount): Thread()
{
    this->counter = 0;
...
SemaphoreConsumer::SemaphoreConsumer(Semaphore *semaphore, unsigned int maxCount):Thread()
{
    this->semaphore = semaphore;
    this->maxCount = maxCount;
}
